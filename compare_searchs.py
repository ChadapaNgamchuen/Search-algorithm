...
def reset_searches():
    ...
    # Generate a list with random numbers and include 88, then sort it for binary search
    numbers = random.sample(range(1, 100), 31) + [88]
...
def update_searches(dt):
    ...
    if not binary_found and binary_left <= binary_right:
        binary_mid = (binary_left + binary_right) // 2
        if numbers[binary_mid] == 88:
            binary_found = True
        elif numbers[binary_mid] < 88:
...
def on_draw():
    ...
    label_font_size = int(box_size * 0.5)  # Font size based on box size
    ...
    for i, number in enumerate(numbers):
        ...
        label = pyglet.text.Label(str(number), font_size=label_font_size, x=x + box_size/2, y=y_linear + box_size/2, anchor_x='center', anchor_y='center', color=(255, 0, 0, 255), batch=batch)
        label.draw()
        label = pyglet.text.Label(str(number), font_size=label_font_size, x=x + box_size/2, y=y_binary + box_size/2, anchor_x='center', anchor_y='center', color=(255, 0, 0, 255), batch=batch)
        label.draw()
